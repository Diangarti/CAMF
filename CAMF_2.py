...
class CAMF():
	def __init__(self, weights=None, img_height = 64, img_width = 64, w = True):
		# Input shape
		self.channels = 1
	...
	def build_detector(self):
		...
		def residual_dense_block(layer_input, filters,name = None):
			"""Residual block described in paper"""
			d = Conv2D(filters, kernel_size=3, strides=1, padding='same')(layer_input)
			d = BatchNormalization(momentum=0.8)(d)
			d = LeakyReLU(alpha=0.2)(d)
			d1 = Concatenate()([d, layer_input])
			d = Conv2D(filters, kernel_size=3, strides=1, padding='same', name = 'conv_'+name)(d1)
			d = BatchNormalization(momentum=0.8)(d)
			d = LeakyReLU(alpha=0.2)(d)
			d2 = Concatenate()([d, d1, layer_input])
			d = Conv2D(filters, kernel_size=1, strides=1, padding='same')(d2)
			#d = LeakyReLU(alpha=0.2)(d)
		...
		r = []
		r.append(residual_dense_block(inp, 64, name = 'r0'))
		for i in range(self.n_residual_blocks - 1):
			r.append(residual_dense_block(r[i], 64, name = 'r'+str(i+1)))

		# Post-residual block
		R = Concatenate()([inp, r[0], r[1], r[2], r[3]])
		d = Conv2D(64, kernel_size=1, strides=1, kernel_regularizer=regularizers.l2(0.01),padding='same')(R)
	...
	def train(self, epochs, batch_size=128, test_interval=20):
		...
		for epoch in range(epochs):
			...
			X_train, y_train = self.data_loader.load_data(samples_per_class = 128,patch_size = 64, mode = 'train')
			X_test, y_test = self.data_loader.load_data(samples_per_class = 128, patch_size = 64, mode = 'test')
			...
			if epoch % test_interval == 0:
				...
				self.detector.save_weights('./2class_dectector_weights.h5')
				plt.figure(100)
				plt.plot(loss)
				plt.plot(accuracy)
				...
				with open('log_2_64class.csv', 'w') as writeFile:
...
if __name__ == '__main__':
    camf = CAMF(img_height = 64, img_width = 64, w = False)
    camf.train(epochs=70, batch_size=128, test_interval=1)
    camf = CAMF(img_height = 64, img_width = 64, weights = '2class_dectector_weights.h5')
    camf.test(name = None )
